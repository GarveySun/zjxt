...
<form method="post" name="form1">
  <table align=center border="1" cellspacing="0" cellpadding="1" bordercolorlight="#CCCCCC" bordercolordark="#FFFFFF" bgcolor="#F2FDFD" width="700">
    <tr bgcolor="#FF8080">
      <td colspan=4 align="center" class=topic>有关部门核实情况</td>
    </tr>
...
%>
    ...
    <%if rs("department") = session("department") then%>
      ...
      <td><textarea name="bmcontent"cols="64" rows="7" ></textarea> 签章：<input type="text" name="bmsign" value="<%=session("name")%>" size="8" />
       </td>
    </tr>
         <%if rs("suggestion_zrbm2")<>"无" then%>
        <tr bgcolor="#FFFFFF" class=small>
      <td align=center><%=rs("over_char2")%>意见</td>
      <td><textarea cols="64" rows="4"><%=rs("suggestion_zrbm2")%></textarea>
       签章：
	...
	<tr bgcolor="#FFFFFF" class=small>
      <td align=center><%=rs("department")%>意见</td>
      <td><textarea cols="64" rows="4"><%=rs("suggestion_zrbm")%></textarea>
       签章：
    ...
    <tr bgcolor="#FFFFFF" class=small>
      <td align=center><%=rs("over_char2")%>意见</td>
      <td><textarea name="bmcontent"cols="64" rows="7" ></textarea>
       签章：
          <input type="text" name="bmsign" value="<%=session("name")%>" size="8" readonly />
       </td>
    </tr>
    <%end if%>
    ...
    <tr bgcolor="#ffefbf">
      <td align=center colspan=2><input type="submit" value="审 核" name="send" onClick="form1.action='/asp/step_bm/JK_editNews_bmchushen_tj.asp?newsid=<%=rs("newsid")%>&again=0';return checkvalue();">
        <input type="submit" value="二次审核" name="again" onClick="form1.action='/asp/step_bm/JK_editNews_bmchushen_tj.asp?newsid=<%=rs("newsid")%>&again=1';return checkvalue();">
