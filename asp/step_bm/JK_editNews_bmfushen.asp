...
function checkvalue()
{
	if(document.form1.bmcontent.value=="")
	{
		alert("请您填写复审意见。");
		return false;
	}
...
<form method="post" name="form1">
  <table align=center border="1" cellspacing="0" cellpadding="1" bordercolorlight="#CCCCCC" bordercolordark="#FFFFFF" bgcolor="#F2FDFD" width="700">
    <tr bgcolor="#FF8080">
      <td colspan=4 align="center" class=topic>相关部门二次审核</td>
    </tr>
...
<%end if%>     
    ...
    <%if rs("department") = session("department") then%>
        <tr bgcolor="#FFFFFF" class=small>
      <td align=center><%=rs("department")%>意见</td>
      <td><textarea name="bmcontent"cols="64" rows="7" class="necessary"><%=rs("suggestion_zrbm")%></textarea> 签章：<input type="text" name="bmsign" value="<%=rs("zrbm_name")%>" size="8" /><div style="color:#666">请填写复审意见</div>
       </td>
    </tr>
         <%if rs("suggestion_zrbm2")<>"无" then%>
        <tr bgcolor="#FFFFFF" class=small>
      <td align=center><%=rs("over_char2")%>意见</td>
      <td><textarea cols="64" rows="4" readonly><%=rs("suggestion_zrbm2")%></textarea>
       签章：
	...
	<tr bgcolor="#FFFFFF" class=small>
      <td align=center><%=rs("department")%>意见</td>
      <td><textarea cols="64" rows="4" readonly><%=rs("suggestion_zrbm")%></textarea>
       签章：
    ...
    <tr bgcolor="#FFFFFF" class=small>
      <td align=center><%=rs("over_char2")%>意见</td>
      <td><textarea name="bmcontent"cols="64" rows="7" class="necessary"><%=rs("suggestion_zrbm2")%></textarea>
       签章：
          <input type="text" name="bmsign" value="<%=rs("zrbm_name2")%>" size="8" readonly/>
          <div style="color:#666">请填写复审意见</div>
       </td>
    </tr>
    <%end if%>
    <tr bgcolor="#ffefbf">
      <td align=center colspan=2><input type="submit" value="审 核" name="send" onClick="form1.action='/asp/step_bm/JK_editNews_bmfushen_tj.asp?newsid=<%=rs("newsid")%>';return checkvalue();">
