...
<table cellspacing="0" cellpadding="0" bordercolorlight="#dfdfdf" bordercolordark="#ffffff" align="center"  width="800">
  <tr style="background-color:#9FF"> 
    <td align="center" class="topic" colspan="8" ><strong>人力资源部审核单据</strong>（单击以查看明细）</td>
  </tr>
...
%>
  <tr class="small" align="center"> 
    ...
    <%if session("seecheck")="查看" then%>
     ...
     <a href="/asp/step_hr/JK_editNews_hrchushen.asp?newsid=<%=rs("newsid")%>">初审</a>
	     <%else%><a href="/asp/step_hr/JK_editNews_hrfushen.asp?newsid=<%=rs("newsid")%>">二次审核</a>
		 <%end if%>
   </td>
  </tr>
  <%
